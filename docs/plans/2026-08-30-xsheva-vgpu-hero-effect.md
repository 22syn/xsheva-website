...
- No test infra exists at all today (`scripts/smoke-test.mjs` is a static `dist/index.html` string-match, no browser, no Playwright/Puppeteer in `devDependencies`). CI **does** exist (`.github/workflows/ci.yml`, a reusable `22syn/ci-templates` workflow) and already runs `npm run test` + lint + build on every PR — task 1.5's `hero-fx.mock-test.mjs` addition rides along automatically since it's Node-only (no GPU, no browser). Task 2.2's Playwright suite and 2.3's Lighthouse comparison are deliberately kept **out** of `ci.yml` (Chromium install + a real browser run is heavier than this reusable workflow is set up for) — that's a choice, not a fallback forced by "no CI to add it to." This plan adds the first browser-based test this repo has ever had — keep it to exactly the assertions the spec's success criteria need, not a general test-suite buildout.
...
- [x] 1.5  Wire the mock test into the existing test command
      Files: `xsheva/package.json`
      What: `"test": "vite build && node scripts/smoke-test.mjs && node scripts/hero-fx.mock-test.mjs"`
      Verify: `cd xsheva && npm run test` exits 0 (all three checks run in sequence). **Confirmed passing** — build succeeded (including the `.wgsl` → JS transform via `wgslVitePlugin`, visible as `dist/assets/hero-grid-*.js` in the build output), smoke test passed, hero-fx mock test passed.
      **Open risk (flag before merging):** `ci.yml` runs `npm run test` on every PR via `22syn/ci-templates`'s reusable `node-ci.yml`, which runs on a standard GitHub-hosted `ubuntu-latest` runner — headless, no physical GPU. `vgpu/node`'s native WebGPU binding (the `webgpu` package, Dawn-backed) rendered correctly on this machine (macOS/Metal), but whether it falls back to a software rasterizer (e.g. Mesa llvmpipe) on a GPU-less Linux CI runner, or fails to init a device at all, is **unverified** — this repo has no CI run to test against yet. First real PR touching this code should be watched closely for a CI failure in `npm run test`; if it fails, the fallback is to skip this specific check in CI (env-gate it) and keep it as a local/pre-push check only, since the mock adapter cannot substitute (see above).
